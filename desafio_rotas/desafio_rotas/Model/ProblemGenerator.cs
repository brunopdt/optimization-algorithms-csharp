namespace desafio_rotas.Model
{
    public class ProblemGenerator
    {
        ...
        * Gerador de rotas aleatórias para testes de FPAA. Recebe como parâmetros a
            ...
            */
        public static List<int[]> generateRoutes(int quantRoutes, int sizeSet,
        double dispersion)
        {
            List<int[]> setTest = new(sizeSet);
            int size_max = (int)Math.Ceiling(SIZE_BASE * (1 + dispersion));
            for (int i = 0; i < sizeSet; i++)
            {
                int[] routes = new int[quantRoutes];
                for (int j = 0; j < quantRoutes; j++)
                {
                    routes[j] = random.Next(SIZE_BASE, size_max);
                }
                setTest.Add(routes);
            }
            return setTest;
